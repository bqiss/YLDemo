...
+ (instancetype)searchViewControllerWithHotSearches:(NSArray<NSString *> *)hotSearches searchBarPlaceholder:(NSString *)placeholder
{
    ...
    UITextField *searchTextField = [searchVC.searchBar valueForKey:@"_searchField"];
    searchTextField.font = [UIFont systemFontOfSize:12];
    [searchVC.searchBar bringSubviewToFront:searchVC.view];
    searchVC.searchBar.hidden = NO;
    return searchVC;
}
...
- (void)setup
{
    ...
    self.navigationItem.rightBarButtonItem.tintColor = SecoTextColor;
...
//    titleView.backgroundColor = HEXCOLOR(0xEFEFEF);
    ...
    [self.searchBar setImage:[UIImage imageNamed:@"icon_search_blue"]forSearchBarIcon:UISearchBarIconSearch state:UIControlStateNormal];
...
- (void)emptySearchHistoryDidClick
{
    ...
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"" message:[NSString stringWithFormat:@"清除历史记录后不可找回"] preferredStyle:UIAlertControllerStyleAlert];
